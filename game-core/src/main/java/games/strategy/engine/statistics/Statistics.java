...
class Statistics {
  ...
  static class BattleStatistics {
    ...
    private Map<GamePlayer, Map<UnitType, Integer>> casualtiesPerPlayerPerUnitType =
        new LinkedHashMap<>();
    /**
     * Map<"Battle", "initial tuv of participating units attacker + defender"> Battle = <battleTime,
     * battleSite, attacker>
     */
  ...
  private static class HistoryTraverseMeasurer {
    ...
    private boolean isEventChildAndUserObjectContains(
        HistoryNode node, String userObjectContainsCondition) {
    ...
    void lookAt(TreeNode treeNode) {
      ...
      if (isEventChildAndUserObjectContains(node, "Battle casualty summary:")) {
        ...
        for (TripleAUnit killedUnit : killed) {
          workingOn.casualtiesPerPlayerPerUnitType.putIfAbsent(
              killedUnit.getOwner(), new LinkedHashMap<>());
          workingOn
              .casualtiesPerPlayerPerUnitType
              .get(killedUnit.getOwner())
              .putIfAbsent(killedUnit.getType(), 0);
          workingOn
              .casualtiesPerPlayerPerUnitType
              .get(killedUnit.getOwner())
              .compute(killedUnit.getType(), (tripleAUnit, integer) -> integer + 1);
        }
      }
      if (isEventChildAndUserObjectContains(node, " attack with ")
        ...
        Triple<Round, Territory, GamePlayer> battle =
            Triple.of(
        ...
        Optional<Integer> tuv =
            units.stream()
                .collect(Collectors.groupingBy(Unit::getOwner))
                .entrySet()
                .stream()
                .map(
                    gamePlayerListEntry ->
                        TuvUtils.getTuv(
                            gamePlayerListEntry.getValue(),
                            getUnitCostMap().get(gamePlayerListEntry.getKey())))
                .reduce((i, j) -> i + j);
  ...
  static BattleStatistics calculateBattleStatistics(GameData gameData) {
    ...
    Enumeration<TreeNode> treeNodeEnumeration =
        ((HistoryNode) gameData.getHistory().getRoot()).breadthFirstEnumeration();
    ...
    return BattleStatistics.builder()
        ...
        .casualtiesPerPlayerPerUnitType(
            historyBasedMeasurer.getStatistics().getCasualtiesPerPlayerPerUnitType())
        .build();
  }

  private static final Map<Statistic, IStat> defaultGameStatisticOverRoundsMappings =
      Map.of(
  ...
  private static Map<Statistic, IStat> createGameStatisticsOverRoundsMapping(
      List<Resource> resources) {
    Map<Statistic, IStat> result = new HashMap<>(defaultGameStatisticOverRoundsMappings);
    for (Resource resource : resources) {
      result.putIfAbsent(
          new Statistic.ResourceStatistic(resource), new AbstractStatPanel.ResourceStat(resource));
    }
    return result;
  }

  static Map<Statistic, Map<String, double[]>> calculateGameStatisticsOverRounds(
      GameData gameData) {
    Map<Statistic, IStat> statisticsMapping =
        createGameStatisticsOverRoundsMapping(gameData.getResourceList().getResources());
      ...
      statisticsMapping
          ...
          .forEach(
              (statistic) -> {
                ...
                alliances.forEach(
                    alliance -> result.get(statistic).put(alliance, new double[rounds.size()]));
              });
    }
    ...
    for (Round round : rounds) {
      ...
      for (Map.Entry<Statistic, IStat> statistic : statisticsMapping.entrySet()) {
        ...
        for (GamePlayer player : players) {
          result.get(statistic.getKey()).get(player.getName())[roundIndex] =
              statistic.getValue().getValue(player, gameData);
        }
        for (String alliance : alliances) {
          result.get(statistic.getKey()).get(alliance)[roundIndex] =
              statistic.getValue().getValue(alliance, gameData);
        }
      }
    }
