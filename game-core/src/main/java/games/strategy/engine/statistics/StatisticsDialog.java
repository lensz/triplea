...
public class StatisticsDialog extends JPanel {
  ...
  public StatisticsDialog(GameData gameData) {
    Map<Statistic, Map<String, double[]>> statisticsData =
        Statistics.calculateGameStatisticsOverRounds(gameData);

    JLabel numberOfRounds =
        new JLabel(
            "Number of rounds: " + Statistics.calculateNumberOfRounds(gameData.getHistory()));
    ...
    JTabbedPane jTabbedPane = new JTabbedPane();
    {
      ...
      jTabbedPane.addTab(
          ...
          createSimpleStatPanel(
              gameData, statisticsData, "TUV Overview", "TUV", Statistic.PredefinedStatistic.TUV));
      jTabbedPane.addTab(
      ...
      jTabbedPane.addTab(
          "VPs over time",
          createSimpleStatPanel(
              gameData, statisticsData, "VP Overview", "VPs", Statistic.PredefinedStatistic.VP));
      gameData
          .getResourceList()
          .getResources()
          .forEach(
    ...
    jTabbedPane.addTab(
        "Battle statistics",
        new BattleStatisticsTab(Statistics.calculateBattleStatistics(gameData)));
