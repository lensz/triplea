...
class BattleStatisticsTab extends JPanel {
  ...
  private JTextArea createTuvLossesPanel(Statistics.BattleStatistics statistics) {
    return new JTextArea(
        String.format(
            "TUV lost by attackers:\t%.0f\nTUV lost by defenders:\t%.0f",
            statistics.getTotalUnitsLostAttacker(), statistics.getTotalUnitsLostDefender()));
  }
  ...
  private XChartPanel<PieChart> createBattleTypesChart(Statistics.BattleStatistics statistics) {
    PieChartBuilder pieChartBuilder =
        new PieChartBuilder().title("Battle Types").width(50).height(50);
  ...
  private JScrollPane createTablePanel(
      ...
      BiFunction<Integer, Integer, Object> getValueAtXY,
      String title) {
  ...
  private JScrollPane createMostContestTerritoryPanel(Statistics.BattleStatistics statistics) {
    List<Map.Entry<String, Double>> battleSitesByNumberOfBattles =
        statistics.getBattleSiteCount().entrySet().stream()
  ...
  private JScrollPane createBiggestBattlePanel(Statistics.BattleStatistics statistics) {
    List<Map.Entry<Triple<Round, Territory, GamePlayer>, Double>> tuvPerBattle =
        statistics.getInitialTuvBattle().entrySet().stream()
            .sorted(Comparator.comparing(Map.Entry::getValue, Comparator.reverseOrder()))
            .collect(Collectors.toList());
